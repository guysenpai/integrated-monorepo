...
describe('getLastVersion', () => {
  ...
  it('should compute current version without a matching prerelease preid', async () => {
    mockGitSemverTags.mockResolvedValue(['my-lib-2.1.0-add-feature.5', 'my-lib-2.0.0', 'my-lib-1.0.0']);

    const tag = await lastValueFrom(getLastVersion({ tagPrefix, preid: 'new-feature' }));
  ...
  it('should compute current version from previous semver prerelease tag with corresponding preid', async () => {
    ...
    const tag = await lastValueFrom(getLastVersion({ tagPrefix }));
    const tagWithPreidFeat = await lastValueFrom(getLastVersion({ tagPrefix, preid: 'add-feature' }));
    const tagWithPreidFix = await lastValueFrom(getLastVersion({ tagPrefix, preid: 'fix-bug' }));
  ...
  it('should compute current version from previous semver release tag', async () => {
    mockGitSemverTags.mockResolvedValue(['my-lib-2.1.0-beta.0', 'my-lib-2.0.0', 'my-lib-1.0.0']);

    const tag = await lastValueFrom(getLastVersion({ tagPrefix, includePrerelease: false }));
