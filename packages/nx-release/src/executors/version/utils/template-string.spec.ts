...
describe('createTemplateString', () => {
  ...
  it('should resolve nothing', () => {
    expect(createTemplateString('test string that have test1 and ${nothing} in it', testContext)).toBe(
      'test string that have test1 and ${nothing} in it'
    );
  });

  it('should resolve ${test1} placeholders', () => {
    expect(createTemplateString('test string with ${test1}, when ${test1} repeat itself', testContext)).toBe(
      'test string with xxx, when xxx repeat itself'
    );
  });

  it('should resolve ${test1} and ${test2} placeholders', () => {
    expect(createTemplateString('test string with ${test1} and ${test2}', testContext)).toBe(
      'test string with xxx and yyy'
    );
  });
...
describe('coerce', () => {
  ...
  it('should handle multiple keys', () => {
    expect(coerce(createTemplateString('${num}', { num: 42, bool: true }))).toBe(42);
  });

  it('should handle multiple interpolations', () => {
    expect(coerce(createTemplateString('${num} ${bool}', { num: 42, bool: true }))).toBe('42 true');
  });
});
